...
void CardPlayedParticleEffectGameAction::VInitAnimation()
{
    ...
    auto activeScene = CoreSystemsEngine::GetInstance().GetActiveSceneManager().FindScene(game_constants::IN_GAME_BATTLE_SCENE);
    
    assert(!lastPlayedCardSoWrapper->mCardData->mParticleEffect.isEmpty());
    
    CoreSystemsEngine::GetInstance().GetParticleManager().CreateParticleEmitterAtPosition(lastPlayedCardSoWrapper->mCardData->mParticleEffect, lastPlayedCardSoWrapper->mSceneObject->mPosition, *activeScene, PARTICLE_SCENE_OBJECT_NAME);
}
...
{
    auto activeScene = CoreSystemsEngine::GetInstance().GetActiveSceneManager().FindScene(game_constants::IN_GAME_BATTLE_SCENE);
    if (activeScene->FindSceneObject(PARTICLE_SCENE_OBJECT_NAME))
    {
