...
void main()
{
    ...
    vec2 n0Uv = vec2(final_uv_x * 1.4f  + 0.01f,  final_uv_y        + time * 0.69f);
    vec2 n1Uv = vec2(final_uv_x * 0.5f  - 0.033f, final_uv_y * 2.0f + time * 0.12f);
    vec2 n2Uv = vec2(final_uv_x * 0.94f + 0.02f,  final_uv_y * 3.0f + time * 0.61f);
    
    float n0A = (texture(tex, n0Uv).a - 0.5f) * 2.0f;
    float n1A = (texture(tex, n1Uv).a - 0.5f) * 2.0f;
    float n2A = (texture(tex, n2Uv).a - 0.5f) * 2.0f;
    
    float noiseA = clamp(n0A + n1A + n2A, -1.0, 1.0);
    
    vec2 n0UvB = vec2(final_uv_x * 0.7f - 0.01f,   final_uv_y * 1.0f + time * 0.27f);
    vec2 n1UvB = vec2(final_uv_x * 0.45f + 0.033f, final_uv_y * 1.9f + time * 0.61f);
    vec2 n2UvB = vec2(final_uv_x * 0.8f - 0.02f,   final_uv_y * 2.5f + time * 0.51f);
    
    float n0B = (texture(tex, n0UvB).a - 0.5f) * 2.0f;
    float n1B = (texture(tex, n1UvB).a - 0.5f) * 2.0f;
    float n2B = (texture(tex, n2UvB).a - 0.5f) * 2.0f;
    float noiseB = clamp(n0B + n1B + n2B, -1.0f, 1.0f);
    
    vec2 final_noise = vec2(noiseA, noiseB);
    float perturb = (1.0f - final_uv_y) * 0.35f + 0.02f;
